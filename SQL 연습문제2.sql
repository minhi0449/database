...
insert into `bank_customer` values ('910912-1000005','신사임당',1,'010-1234-1005','서울시 강릉시');
...
select * from `bank_account` where `a_balance` = (select max(`a_balance`)
												  from `bank_account`
...
#실습 2-20
#조건이 생각나야 함 (구글링 하던가 해라) 예. mysql count() 조건 설정하는 법
#count if() 조건 검색 해보기
select
